defmodule M do
  ...
  def directional_to_directional([from, to], cost) do
    from_coords = directional_key_to_coords(from)
    to_coords = directional_key_to_coords(to)

    move_instructions({from_coords, to_coords}, cost, :directional) ++ ["A"]
  end
end
...
directions_2 =
  ...
  |> Enum.map(fn code ->
    ...
    |> Enum.chunk_every(2, 1, :discard)
    |> Enum.map(fn [from, to] ->
      M.directional_to_directional([from, to], cost)
    end)
    |> List.flatten()
  end)
